...
- Current asset version is `20260622-activation-readiness`.
...
- The pilot write gate last passed on 2026-06-22 with 11 live QA submissions, including dedicated pilot feedback routing, inbox delivery required, live event writes, media checks, browser-flow coverage, signal-pass export coverage, and protected operator-report verification. QA run ID: `20260622002939`. The live event matrix includes fan-reaction reporting for the media retention loop, and the gate reconciles exact write-run IDs back through the protected report.
