...
- Current live asset version: `20260630-page-view-reporting`
...
- Pilot write gate last passed on 2026-06-30 with 11 live capture-intent submissions, including dedicated pilot feedback routing, inbox delivery required, 12 live event writes, live media-source checks, live browser-flow coverage, signal-pass export coverage, consented page-view reporting, protected activation-readiness reporting, and protected operator-report verification. QA run ID: `20260630170452`. Asset version: `20260630-page-view-reporting`. Live event matrix coverage includes fan-reaction and release-room retention reporting for the media retention loop, and the gate reconciles exact write-run IDs back through the protected report.
...
Use `node tools/report-open-approvals.mjs` for a no-secret live operator view of every remaining approval, including whether each item blocks public launch or is automation/private workflow follow-up. Validate the approval view with `node tools/qa-open-approvals.mjs`.
...
Inbox delivery was confirmed by the 2026-06-30 pilot write gate, QA run ID `20260630170452`, with 11 live capture intents, 12 event writes, and inbox delivery required. Current pilot evidence asset version: `20260630-page-view-reporting`. Re-run this only if the sender domain, provider secret, Functions deployment, or final release evidence freshness changes:
