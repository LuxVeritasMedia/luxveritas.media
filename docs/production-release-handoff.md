...
- Current live asset version: `20260620-brand-house-rail`
...
- Pilot write gate last passed on 2026-06-20 with 11 live capture-intent submissions, including pilot feedback, inbox delivery required, live event writes, live media-source checks, live browser-flow coverage, and protected operator-report verification. QA run ID: `20260620235930`. Live event matrix coverage now includes fan-reaction reporting for the media retention loop, and the gate now reconciles exact write-run IDs back through the protected report.
