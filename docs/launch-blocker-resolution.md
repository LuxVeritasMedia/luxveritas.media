...
For GitHub Actions deploy recovery, use `FIREBASE_CI_TOKEN` only with a Firebase CLI token generated by `firebase login:ci`. The workflow preserves Workload Identity as the default path and uses the CI token only as the Firebase CLI fallback.
