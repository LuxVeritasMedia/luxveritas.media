...
- Live build version: `20260704-portal-access-path`.
...
- Inbox provider closeout is resolved: the 2026-07-05 pilot write gate confirmed 11 capture intents with inbox delivery required.
- Pilot write gate last passed on 2026-07-05 with live form writes, 13 event writes, inbox delivery required, dedicated pilot feedback routing, fan-reaction reporting, release-room retention reporting, consented page-view reporting, signal-pass export coverage, protected activation-readiness reporting, and post-write reconciliation through the protected operator report. QA run ID: `20260705114236`. Asset version: `20260704-portal-access-path`.
...
Status: closed on 2026-06-17. Latest live revalidation passed on 2026-07-03 via pilot write gate `20260703140813`. Keep the steps below as revalidation guidance if the sender domain, provider secret, or Functions deployment changes.
...
For GitHub Actions deploy recovery, use `FIREBASE_CI_TOKEN` only with a Firebase CLI token generated by `firebase login:ci`. The workflow preserves Workload Identity as the default path and uses the CI token only as the Firebase CLI fallback. `node tools/qa-firebase-deploy-auth.mjs` is the fail-fast preflight for this blocker, and `node tools/setup-firebase-ci-token.mjs` is the no-file helper for storing the token in GitHub Actions and rerunning Hosting.
