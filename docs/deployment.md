...
GitHub Actions deploys use Google Workload Identity as the no-key default path. If Firebase CLI rejects that auth path, set the GitHub Actions secret `FIREBASE_CI_TOKEN` with a token generated by `firebase login:ci` from an approved Lux Veritas Firebase account. Do not use a Google access token for this secret, and do not commit the token to the repo.
