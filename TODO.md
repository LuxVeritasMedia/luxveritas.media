...
- [x] Add protected retention-path summaries for fan-flywheel and brand-house clicks.
...
- [x] Add hidden-input Resend activation terminal runner and QA so inbox keys stay out of shell history.
...
- [ ] Grant `github-actions-firebase@lux-veritas-media.iam.gserviceaccount.com` `iam.serviceAccounts.ActAs` / Service Account User on `lux-veritas-media@appspot.gserviceaccount.com` using `docs/functions-deploy-iam-repair.md`, then rerun the manual Functions workflow; verify with `node tools/qa-functions-deploy-readiness.mjs`. Local Functions deploy already made the dedicated `pilot_feedback` server route live. Explicit project-owner approval is required before this IAM mutation.
...
- [x] Configure and verify email provider runtime secret `RESEND_API_KEY` plus approved sender domain for inbox notification after Firestore capture. 2026-06-21 pilot write gate confirmed 11 capture intents with inbox delivery required.
...
- [x] Add Google Workspace first-activation packet with approval fields, dry-run command, live acceptance checks, and Firebase rollback.
- [x] Add Google Workspace approval closeout rehearsal QA for owner approval, activation dry-run, version lock, and rollback evidence.
...
- [x] Render no-secret action inventory coverage in protected operator reporting.
